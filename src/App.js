...
export default function App() {
  ...
  const handleGenerate = async () => {
    if (!linkedinUrl || !userProfile) { alert("Fill in LinkedIn URL and your profile!"); return; }
    setLoading(true);
    setMessage("Scraping LinkedIn and writing email...");
    try {
  ...
  const handleSend = async () => {
    if (!resume) { alert("Upload your resume!"); return; }
    if (!hrEmail) { alert("Enter HR email!"); return; }
    if (!gmailToken) { alert("Connect your Gmail first!"); return; }
    setLoading(true); setMessage("Sending email...");
    try {
      ...
      <div style={{ background: "white", borderRadius: 16, padding: 40, width: "100%", maxWidth: 560, boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }}>
        ...
        {step === 1 && (
          <div>
            ...
            <textarea style={{...inputStyle, height: 100, resize: "vertical"}} placeholder="My name is Shourya. Final year IIT BHU. Skills: Python, ML..." value={userProfile} onChange={e => setUserProfile(e.target.value)} />
            <label style={labelStyle}>Your Resume (PDF)</label>
            <input type="file" accept=".pdf" onChange={e => setResume(e.target.files[0])} style={{ display: "block", marginBottom: 24, color: "#444" }} />
            <button style={buttonStyle} onClick={handleGenerate} disabled={loading}>{loading ? message : "✨ Generate Email"}</button>
          </div>
        )}
        ...
        {step === 2 && emailPreview && (
          <div>
            ...
            <input style={inputStyle} value={hrEmail} onChange={e => setHrEmail(e.target.value)} placeholder="Enter HR email if not auto-detected" />
            <label style={labelStyle}>Subject</label>
            <input style={inputStyle} value={emailPreview.subject} onChange={e => setEmailPreview({...emailPreview, subject: e.target.value})} />
            <label style={labelStyle}>Email Body (you can edit)</label>
            <textarea style={{...inputStyle, height: 180, resize: "vertical"}} value={emailPreview.email_body} onChange={e => setEmailPreview({...emailPreview, email_body: e.target.value})} />
            <div style={{ display: "flex", gap: 12 }}>
              <button style={{...buttonStyle, background: "#eee", color: "#333", flex: 1}} onClick={() => setStep(1)}>← Back</button>
              <button style={{...buttonStyle, flex: 2}} onClick={handleSend} disabled={loading}>{loading ? message : "📤 Send Email"}</button>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div style={{ textAlign: "center", padding: "20px 0" }}>
            ...
            <p style={{ color: "#666", marginBottom: 32 }}>Delivered successfully.</p>
            <button style={buttonStyle} onClick={() => { setStep(1); setEmailPreview(null); setLinkedinUrl(""); }}>Send Another</button>
          </div>
        )}
      </div>
    </div>
